...
def check_file(path, min_bytes=100):
    ...
    if p.exists() and p.stat().st_size >= min_bytes:
        ...
        print(f"  [✓] {str(path):<45s} ({size_mb:>7.2f} MB)")
        passed_checks += 1
        return True
    else:
        print(f"  [✗] {str(path):<45s} (MISSING or INVALID)")
        return False
...
if passed_checks == total_checks:
    print("  ✓ ALL PIPELINE STAGES ARE WORKING PROPERLY AND VALIDATED!")
else:
    print("  ⚠ Some files were missing or incomplete. Re-run necessary scripts.")
print(f"{SEP}\n")
